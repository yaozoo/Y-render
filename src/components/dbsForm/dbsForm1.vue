 ...
 * @LastEditTime: 2022-11-17 17:12:34
 * @LastEditors: luyao
 * @Description: 
 * @FilePath: /y-render/src/components/dbsForm/dbsForm1.vue
-->
<template>
  <el-form
    ...
    <el-form-item
      ...
      :label="item.label"
      :key="item.prop"
      :style="{ width: formConfig.formItemWidth }"
      ...
      <el-select
        ...
        <el-option
          ...
          :key="op[item.itemValue || 'key']"></el-option>
      </el-select>
      <!-- 级联选择器 -->
      <el-cascader
        clearable
    ...
    <!-- :style="{ width: formConfig.formItemWidth }" -->
    <div class="btn-box" v-if="formConfig.isHandle">
      <span
      ...
      <span
        ...
        v-if="formConfig.isShowFilter && searchBtnShow">
        <el-icon :size="16" v-if="open"><top /></el-icon>
...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    context.emit("getFormRef", dbsFormRef);
    ...
    function reset() {
      let pageSize =
        props.searchData.hasOwnProperty("pageSize") &&
        props.searchData.pageSize;
    ...
    function showAllSearch() {
      nextTick(() => {
        ...
        if (switchBtnDom && searchFormDom) {
          ...
          if (diffHeight > 0) {
            // switchBtnDom.setAttribute("style", "visibility:visible");
            state.searchBtnShow = true;
          } else {
            // switchBtnDom.setAttribute("style", "visibility:hidden");
